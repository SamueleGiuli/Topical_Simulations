...
!        print*,EGbin(j),Gbin(j), j, k,g
  ...
  SigEavg=dsqrt(sum(Gbin*(EGbin/Gbin-Eavg)**2)/sum(Gbin))/dsqrt(dble(Nused-1))
  Xavg=sum(Xbin)/sum(Gbin)
  SigXavg=dsqrt(sum(Gbin*(Xbin/Gbin-Xavg)**2)/sum(Gbin))/dsqrt(dble(Nused-1))

  
  write(*,*) p, Eavg, SigEavg, Xavg, SigXavg
